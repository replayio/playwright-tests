...
(async () => {
  ...
  await expect(page.locator('[data-testid="SideNav_NewTweet_Button"]')).toBeVisible();
  await expect(page.locator('[data-testid*="tweetTextarea_"][aria-label="Tweet text"]')).toBeVisible();
  await expect(page.locator('[data-testid="Tweet-User-Avatar"] >> nth=0')).toBeVisible();
  ...
  await expect(page.locator('[data-testid="ScrollSnap-List"] :text("Tweets")')).toBeVisible();
  await expect(page.locator('[data-testid="ScrollSnap-List"] :text("Replies")')).toBeVisible();
  await expect(page.locator('[data-testid="ScrollSnap-List"] :text("Media")')).toBeVisible();
  await expect(page.locator('[data-testid="ScrollSnap-List"] :text("Likes")')).toBeVisible();
  ...
  await expect(page.locator('[data-testid="cellInnerDiv"] :text("United States trends")')).toBeVisible();
  
  // Click on 'News' 
  await page.click('[data-testid="ScrollSnap-List"] :text("News")');
  await expect(page.locator('[data-testid="trend"] :text("Trending in Business and finance") >> nth=0')).toBeVisible();
  
  // Click on 'Sports'
  await page.click('[data-testid="ScrollSnap-List"] :text("Sports")');
  await expect(page.locator('[data-testid="trend"] :text("Trending in Sports") >> nth=0')).toBeVisible();
  
  // Click on 'Entertainment'
  await page.click('[data-testid="ScrollSnap-List"] :text("Entertainment")');
  await expect(page.locator('[data-testid="trend"] :text("Trending in Music") >> nth=0')).toBeVisible();
