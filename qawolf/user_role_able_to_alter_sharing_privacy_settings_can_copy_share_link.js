...
(async () => {
  ...
  const privateButton = page.locator(
    '[data-test-id="consoleDockButton"]:has-text("Test Permissions can viewexpand_more")'
  );
  const publicButton = page.locator(
    '[data-test-id="consoleDockButton"]:has-text("Anyone with the link can viewexpand_more")'
  );
  const publicLink = page.locator(
    '[role="menuitem"] >> text=Anyone with the link'
  );
  const privateLink = page.locator(
    '[role="menuitem"] >> text=Members of Test Permissions'
  );
  ...
  expect(copiedLink).toEqual(
    buildUrl("/recording/time-travel--ebf103b3-9b40-4d5b-a9f3-e0c8fe3d4bd5")
  );
