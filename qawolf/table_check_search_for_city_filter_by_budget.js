...
(async () => {
  ...
  const getSliderBounds = getBoundingClientRect(
    selectors.results.filter.slider
  );
  ...
  // set starting price
  await page.mouse.click(bounds.left + Math.round(bounds.width * 0.25), center);

  // set ending price
  await page.mouse.click(bounds.left + Math.round(bounds.width * 0.75), center);
