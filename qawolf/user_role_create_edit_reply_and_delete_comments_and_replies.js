...
(async () => {
  ...
  await expect(page.locator('#app-container span >> text=Your Library')).toBeVisible();
  await page.click('[title="Test Commenters"]');
  await page.click('text=User role commenter test');
  
  // assert replay and comments loaded
  await expect(page.locator('text="User role commenter test"')).toBeVisible( {timeout: 30000});
  ...
  // await page.click(".canvas-comment");
  ...
  // edit comment
  await page.waitForTimeout(5000);
  await page.locator(':text("more_vert"):above(:text("Here is my mispelling"))').first().click();
  ...
  // delete reply
  const reply = page.locator(':text("QA WolfNowmore_vertHere is my reply") button');
  ...
  await page.waitForTimeout(3000);
  await page.locator(':text("more_vert"):above(:text("Here is my updated comment!"))').first().click();
  await deleteCommentsAndReplies(page);
  
  // assert comment and reply deleted
  await page.waitForTimeout(5000);
  await expect(page.locator('text="Here is my updated comment!"')).not.toBeVisible();
