...
(async () => {
  ...
  try {
    ...
    await page.click('[data-test="compute-graph-provider"] :text("Edit profile")');
    await page.fill('[data-test="compute-graph-provider"] [placeholder="Name"]', ogName);
    await page.fill('[data-test="compute-graph-provider"] [placeholder="Bio"]', ogBio);
    await page.hover('[data-test="compute-graph-provider"] .editable-field');
    const [fileChooser] = await Promise.all([
      page.waitForEvent("filechooser"),
  
      // Opens the file chooser.
      await page.click('[data-test="compute-graph-provider"] :text("Update photo")')
    ]);
  ...
  try {
    ...
    await page.fill('[data-test="report-editable-content"][contenteditable="true"]', ogIntroText);
    await page.click(':text("Save")');
  
    while(await page.locator('[data-test="profile-page-tabs"] a .drag-source').count()) {
      await page.click('[data-test="profile-page-tabs"] button:below(:text("Links")) >> nth=0'); // click trash 
      await page.waitForTimeout(2000);
    }
  
    await expect(page.locator(`:text("${ogIntroText}")`)).toBeVisible();
    await expect(page.locator('[data-test="profile-page-tabs"] :text("Add a link")')).toBeVisible();
  }
  ...
  await page.fill('[data-test="compute-graph-provider"] [placeholder="Name"]', newName);
  await page.fill('[data-test="compute-graph-provider"] [placeholder="Bio"]', newBio);
  ...
  const newIntroText = "Hello, I'm google and I know all the answers to your questions.";
  await page.fill('[data-test="report-editable-content"][contenteditable="true"]', newIntroText);
  await page.click(':text("Save")');
