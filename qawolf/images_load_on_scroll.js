...
(async () => {
  ...
  const imgCount1 = await page
    .locator('[class="lazyload-wrapper "] img')
    .count();
  ...
  const totalReplays = Number(
    (await page.innerText(".font-semibold")).split("(")[1].replace(")", "")
  );

  // scroll to bottom
  await page
    .locator(`.border-chrome >> nth=${rowCount - 1}`)
  ...
  await page
    .locator(
      '[href="/recording/time-travel--9b410334-a4d6-48b8-9cf9-116077e1ef37"]'
    )
    .scrollIntoViewIfNeeded();
