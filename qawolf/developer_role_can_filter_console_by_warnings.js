...
(async () => {
  const TEST_NAME = "Developer role can filter console by warnings";
  ...
  await page.uncheck('[data-test-id="FilterToggle-warnings"] #FilterToggle-warnings');
  
  // assert warnings loaded
  await expect(warnings).toHaveCount(0);
  
  // ensure warnings are checked
  await page.check('[data-test-id="FilterToggle-warnings"] #FilterToggle-warnings');
