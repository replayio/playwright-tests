...
(async () => {
  ...
  await page.goto(
    buildUrl("/recording/qa-wolf--ee90e0d1-e786-446d-b889-5659226346df")
  );
  ...
  await expect(
    page.locator("text=Click Open Replay in the dialog shown by your browser")
  ).toBeVisible();
  await expect(
    page.locator(
      "text=Don't have Replay yet? Download it on Mac, Linux, and Windows"
    )
  ).toBeVisible();
  ...
  expect(linuxLink).toEqual(
    "https://static.replay.io/downloads/linux-replay.tar.bz2"
  );
  expect(windowsLink).toEqual(
    "https://static.replay.io/downloads/windows-replay.zip"
  );
