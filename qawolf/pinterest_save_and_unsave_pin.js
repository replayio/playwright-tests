...
(async () => {
  ...
  try {
    await expect(
      page.locator('[data-test-id="board-[DONT DELETE] Save/Unsave Pin"]')
    ).toBeVisible();
  } catch {
    await page.click(
      '[data-test-id="boardActionsButton"] [aria-label="Create a Pin or a board"]'
    );
  ...
  try {
    await page.click(
      '[data-test-id="boardCard-[DONT DELETE] Save/Unsave Pin"]'
    );
  } catch (e) {}
  await page.waitForTimeout(1000);
  ...
  try {
    ...
    await expect(
      page.locator('[data-test-id="pinrep-image"] >> nth=0')
    ).toBeVisible();
  ...
  await expect(
    page.locator('[data-test-id="better-save"] [aria-label="Pin Saved"]')
  ).toBeVisible();
  await page.waitForTimeout(5000);

  // edit saved location from 'profle' to '[DONT DELETE] Save/Unsave Pin' to easily assert the exact pin we saved
  await page.click(
    '[data-test-id="pointer-events-wrapper"] [aria-label="Edit"]'
  );
  await page.waitForTimeout(5000);
  await page.click('[data-test-id="edit-board"] :text("Profile")');
  await page.waitForTimeout(5000);
  await page.click(
    '[data-test-id="board-selection"] :text("[DONT DELETE] Save/Unsave Pin")'
  );
  await page.waitForTimeout(5000);
