...
(async () => {
  ...
  // assert role options
  await expect(
    page.locator('[role="menu"] .permissions-dropdown-item >> text=User')
  ).toBeVisible();
  await expect(
    page.locator('[role="menu"] .permissions-dropdown-item >> text=Developer')
  ).toBeVisible();
  ...
  await expect(
    page.locator("text=Give access to anyone with a qawolf.com email address")
  ).not.toBeVisible();
  ...
  await expect(
    page2.locator("button >> text=Sign in with Google")
  ).toBeVisible();
