...
(async () => {
  ...
  const [page2] = await Promise.all([
    ...
    page.locator('[data-qa="app_launcher__app_card--not_installed"] [aria-label="Easily start a Zoom video meeting directly from Slack."]').click() // Opens a new tab
  ])
  ...
  await page.reload();
  await expect(page.locator('[data-qa="app_launcher__app_card--installed"]')).toContainText("Zoom");
  ...
  await expect(page.locator('[data-qa="app_launcher__app_card--not_installed"] [aria-label="Easily start a Zoom video meeting directly from Slack."]')).toBeVisible();
