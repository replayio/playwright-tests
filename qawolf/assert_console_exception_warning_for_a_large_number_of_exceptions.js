...
(async () => {
  ...
  await page.check(
    '[data-test-id="FilterToggle-exceptions"] [type="checkbox"]'
  );

  // assert warning icon is shown
  const warningIcon = page.locator(
    '[data-test-id="ConsoleFilterToggles"] [title="There are too many exceptions. Please focus to a smaller time range and try again."]'
  );
  ...
  // hide exceptions
  await page.click(
    '[data-test-id="FilterToggle-exceptions"] #FilterToggle-exceptions'
  );
