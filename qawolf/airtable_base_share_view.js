...
(async () => {
  ...
  // assert values
  await expect(page2.locator('[data-testid="viewName"]')).toHaveText(
    "Grid view"
  );
  await expect(page2.locator('[data-testid="navBarButtonInBase"]')).toHaveText(
    "Sign up for free"
  );
  await expect(
    page2.locator('[aria-label="Name column header (Single line text field)"]')
  ).toBeVisible();
  await expect(
    page2.locator('[aria-label="Notes column header (Long text field)"]')
  ).toBeVisible();
  await expect(
    page2.locator('[aria-label="Attachments column header (Attachment field)"]')
  ).toBeVisible();
  await expect(
    page2.locator('[aria-label="Status column header (Single select field)"]')
  ).toBeVisible();
