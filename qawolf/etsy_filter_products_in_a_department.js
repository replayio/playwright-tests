...
(async () => {
  ...
  await expect(
    page.locator('[aria-label="Remove Black Filter"]')
  ).toBeVisible();

  // grab two listings - can't grab first two because they're the sponsored ones
  const firstListingName = (
    await page.innerText(".listing-link >> nth=5")
  ).split(" |")[0];
  ...
  let tempName1 = (await page.innerText(".listing-link >> nth=5")).split(
    " |"
  )[0];
  let tempName2 = (await page.innerText(".listing-link >> nth=6")).split(
    " |"
  )[0];
