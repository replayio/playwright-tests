...
(async () => {
  ...
  try {
    ...
    await expect(
      page.locator('h1:has-text("QA Wolf Test User")')
    ).toBeVisible();
  }
  ...
  // - Experience
  await expect(
    page.locator('span:text-is("Experience") >> nth=0')
  ).toBeVisible();
  // - Interests
  await expect(
    page.locator('span:text-is("Interests") >> nth=0')
  ).toBeVisible();
  ...
  await expect(
    page.locator(
      '[data-test-modal-container=""] #profile-edit-form-page-header'
    )
  ...
  await expect(
    page.locator("text=Houston, Texas, United States ")
  ).toBeVisible();
  await expect(
    page.locator(
