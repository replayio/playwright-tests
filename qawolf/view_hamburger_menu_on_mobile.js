...
(async () => {
  ...
  // open menu
  await page.click(
    ".button_md____TNT:above(:text('The time-travel debugger from the future.'))"
  );
  await page.waitForTimeout(1000); // give menu time to open

  // find location of each link
  var docsLink = page.locator(
    '.header_menu-inner__zV_GW [href="https://docs.replay.io"]'
  );
  ...
  var hiringLink = page.locator(
    '.header_menu-inner__zV_GW [href="/about#jobs"]'
  );
  var hiringLinkBounds = await hiringLink.first().boundingBox();
  var loginButton = page.locator(
    '.header_menu-inner__zV_GW [href="https://app.replay.io/"]'
  );
  var loginButtonBounds = await loginButton.first().boundingBox();
  var discordContainer = page.locator(
    '.header_menu-inner__zV_GW [href="/discord"]'
  );
